...
The biggest workflow shift at the table came from [Zed's parallel agents](https://zed.dev/parallel-agents) running over the [Agent Client Protocol](https://zed.dev/acp) (worth distinguishing the two: ACP is the [open Apache-licensed protocol](https://github.com/agentclientprotocol/agent-client-protocol) Zed launched in [August 2025](https://zed.dev/blog/bring-your-own-agent-to-zed) for connecting any agent to any editor; parallel agents is the orchestration UI [shipped April 22, 2026 in Zed v0.233.5](https://zed.dev/blog/parallel-agents) that runs multiple agents in one window). One software architect described it as a step-change: bring your own Claude or Codex subscription, route it through Zed's agent panel, and dispatch threads across multiple codebases at once. Each thread can keep its own persistent terminal, file explorer, git, and profiler — none of the rot you get juggling tmux tabs against the desktop apps (per [Zed's parallel agents page](https://zed.dev/parallel-agents), the isolation primitive is actually opt-in Git worktrees per thread, with the new Threads Sidebar scoping which folders and repos each agent can touch).

The same architect tried [Claude's desktop app](https://claude.com/download) but kept hitting crashes and non-sticky terminals. Codex got higher marks for stability but felt verbose — one attendee cited reports that Codex burns roughly 4x more tokens than Claude for equivalent work (per [morphllm's benchmarks](https://www.morphllm.com/comparisons/codex-vs-claude-code), Claude Code uses ~4x more tokens than Codex). The compromise: stay in the IDE, push out parallel agents, and only context-switch when you genuinely need to write code by hand. Some changes are still 30 seconds of human typing — faster than any agent can boot.

### The Context Lake Pattern

One attendee is building what [Port.io now calls a "Context Lake"](https://www.port.io/glossary/context-lake): a unified surface that pulls in logs, metrics, traces, build costs, security warnings, customer requests, and meeting transcripts — all queryable by humans and agents through a single MCP (per [Port's own definition](https://www.port.io/glossary/context-lake), a Context Lake covers four layers: domain knowledge, operational state, engineering metadata, and actions/tools — the attendee's implementation extends that scope to include things like meeting transcripts and customer requests). Every morning, engineers log in and ask "what's going on?" The agent answers from real data: which repos are throwing warnings, which builds are getting expensive, which business metric is plateauing.
...
The visiting founder is building a "cursor for games" — an AI-native dev environment for Unity and Unreal, made by [Playco](https://www.play.co/). The argument: 80% of professional games are built in those editors (GDC's 2024–2025 industry surveys put combined Unity + Unreal share at ~64%), and AI coding tools assume you're working on a website. You can point Claude at Unity code, but the editor work — scene setup, asset wiring, prefabs — is still all manual. The result is that the 8-million-strong professional game-dev population (SlashData counts ~11M game devs worldwide, including hobbyists and indies) is years behind the rest of software on AI adoption, partly because game engineers and web engineers barely cross paths.
...
Same story for EVs. [Korea is the most electric country](https://www.koreaherald.com/article/10580014) he's been to outside China. Japan [still has effectively zero electric cars](https://www.jato.com/resources/news-and-insights/japans-automotive-electrification-trends-2025-h1) and was pursuing hydrogen until last year (Japan's 2030 plan includes 1,000 hydrogen stations alongside 150,000 EV chargers, with a target of 12M tons of hydrogen annually by 2040). The really shocking part wasn't the cars — it was the electric scooters, often 30 years old, hot-wired with what look like car batteries, driven by 80-year-olds through red lights with no plates and no police interest. They work. They're quiet. They're ubiquitous. None of it exists in the West.
...
- [DeepSeek V4](https://api-docs.deepseek.com/news/news260424): Released April 24. The 3D-printing software team runs it for non-sensitive workloads — "infinitely better than V3, 40x cheaper than Codex per token."
